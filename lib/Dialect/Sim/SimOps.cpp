...
LogicalResult DPIFuncOp::verify() {
  if (getNumResults() != 0)
    return emitError() << "non-void function is currently unsupported";
  return success();
}
...
LogicalResult DPICallOp::verify() {
  if (getNumResults() != 0)
    return emitError() << "non-void function is currently unsupported";
  if (!getClock())
    return emitError() << "void function must be called with a clock";
  return success();
}

LogicalResult DPICallOp::verifySymbolUses(SymbolTableCollection &symbolTable) {
  auto referencedOp = dyn_cast_or_null<DPIFuncOp>(
